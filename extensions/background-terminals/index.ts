...
import type { TerminalSnapshot } from "./src/public-domain.ts";
...
import type { TerminalRuntime } from "./src/runtime.ts";

type RuntimeModules = typeof import("./src/runtime.ts") & {
  manager: typeof import("./src/manager.ts");
};
...
export default function (pi: ExtensionAPI) {
  ...
  const loadRuntimeModules = () =>
    (runtimeModulesPromise ??= Promise.all([
      ...
      import("./src/manager.ts"),
    ])
      .then(([runtimeModule, manager]) => ({ ...runtimeModule, manager }))
      .catch((error) => {
  ...
  const getRuntime = async () => {
    ...
    return (runtime ??= modules.createTerminalRuntime());
  };
  ...
  const getManager = () => {
    managerPromise ??= (async () => {
      const modules = await loadRuntimeModules();
      const manager = await (await getRuntime()).runPromise(
        modules.manager.TerminalManager,
      );
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const snap = await runtimeModules.runTool(
        await getRuntime(),
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal) {
      ...
      const runtimeModules = await loadRuntimeModules();
      const report = await runtimeModules.runTool(
        await getRuntime(),
        manager.kill(ids),
        {
          signal,
