...
async function awaitWithSignal<T>(
  ...
  if (signal.aborted) throw new AskUserAbortError("Cancelled");

  let onAbort: (() => void) | undefined;
  const aborted = new Promise<never>((_resolve, reject) => {
    onAbort = () => reject(new AskUserAbortError("Cancelled"));
...
export default function askUser(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      try {
        result = await awaitWithSignal(showQuestion, signal);
      } catch (error) {
        if (error instanceof AskUserAbortError || signal?.aborted) {
          return reply(buildAskUserResultMessage({ kind: "cancelled" }));
        }
        throw error;
      }
